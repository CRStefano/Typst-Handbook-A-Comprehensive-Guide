...
Typst è nato già nel 2019 come progetto di tesi di due studenti
dell'Università Tecnica di Berlino, Laurenz Mäule e Martin Haug.
...
Per inserire una linea orizzontale separatrice:
...
Tre trattini `---` consecutivi producono una linea orizzontale
(in realtà è un _em dash_, il trattino lungo, ma in contesto di paragrafo separato
diventa una linea di separazione visiva).
...
=== Immagini da URL remoti

`````typ
// Scarica e incorpora un'immagine da internet
#image("https://typst.app/assets/logo.svg", width: 4cm)
`````

#nota[
  Gli URL remoti funzionano nell'editor online di typst.app.
  Localmente, il compilatore deve avere accesso alla rete.
]
...
//metodo push
#numeri.push(6)// Aggiunge in fondo (gli array di typst non sono mutabili come su python quindi il push funziona in maniera un po' diversa. serve solo per la modifica. per la nuova creazione va usato un nuovo "let")
// Slice (sottoarray)
...
//metodo insert. I dizionari sono parzialmente immutabili, per creazioni ex novo meglio il let precedente.
#persona.insert("email", "mario@esempio.it")
// Le chiavi
...
@inproceedings{typst2023,
 author = {Haug, Martin and Mäule, Laurenz},
